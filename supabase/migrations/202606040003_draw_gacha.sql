-- Server-side gacha draw RPC.
-- The client must never determine draw results.
...
begin
  ...
  values (
    ...
    v_request_id,
    'started',
    'order by gen_random_uuid() with row lock',
    v_random_seed_hash,
  ...
  if not found then
    raise exception '요청이 몰려 추첨에 실패했습니다. 다시 시도해주세요.';
  end if;
